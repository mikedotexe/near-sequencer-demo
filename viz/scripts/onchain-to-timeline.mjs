 ...
 * height, receipt, and log line is pulled from the captured onchain.json.
 * This is the whole point of the capture: every live-rendered scene is
...
function findTraceBlock(ev, name) {
  for (const tx of allCapturedTxs()) {
...
function assembleTimeout() {
  ...
  // usually succeeds from the captured yield tx. Fall back to
  // yield+200 only if the capture somehow missed the post-budget block.
...
function assembleHandoff() {
  ...
  // Timeout fallback: if the capture missed the post-budget block, fake
...
if (events.length === 0) {
  console.error("[translate] no events found in capture; check that the raw/onchain pair belongs to this recipe");
  process.exit(2);
}
...
const timeline = {
  name: scene_name,
  comment: `Generated by onchain-to-timeline.mjs from ${path.basename(rawPath)}; sourced from ${onchain.rpcEndpoints.archival}. Recipe=${recipe}, network=${onchain.network}, captured=${onchain.capturedAt}. Every block height below is the real observed block.`,
  source: {
    recipe,
